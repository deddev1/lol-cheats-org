# Deploy lolcheats.org

Step-by-step guide to deploy the League of Legends Hacks static site to **lolcheats.org** on Cloudflare Pages, configure DNS, and submit to Google Search Console.

## Prerequisites

- Node.js **≥ 22.12.0**
- Cloudflare account with access to **lolcheats.org** DNS
...
3. Configure build settings:
   - **Project name:** `lolscheats` (existing) or create a new project
...
This runs `wrangler pages deploy dist --project-name=lolcheats` (see `wrangler.toml`).

## 3. Custom domain and DNS

Add **lolcheats.org** as the primary custom domain on the Pages project.
...
2. In **Rules** → **Redirect Rules** (or Bulk Redirects), create:
   - **Source:** `www.lolcheats.org/*`
   - **Target:** `https://lolcheats.org/${1}`
   - **Status:** 301

The deployed `functions/_middleware.js` also enforces apex canonical host, legacy domain redirects (`lolcheats.org`, `.net`, `.com`), and legacy path redirects.
...
- `https://lolcheats.org/`
- `https://lolcheats.org/es/`
- `https://lolcheats.org/lol-cheats/`
- `https://lolcheats.org/lol-aimbot/`
- `https://lolcheats.org/sitemap.xml`
- `https://lolcheats.org/robots.txt`

Verify redirects:

- `http://lolcheats.org` → `https://lolcheats.org` (301)
- `https://www.lolcheats.org` → `https://lolcheats.org` (301)
- Legacy domains (e.g. `lolcheats.org`) → `https://lolcheats.org` (301)
- `/sitemap-index.xml` → `/sitemap.xml` (301)
- Legacy paths (e.g. `/fortnite-hacks/`) → League of Legends equivalents (301)

## 5. Google Search Console

1. Go to [Google Search Console](https://search.google.com/search-console).
2. **Add property** → choose **Domain** → enter `lolcheats.org`.
...
4. After verification, open **Sitemaps** and submit:
   ...
   https://lolcheats.org/sitemap.xml
   ```
   Remove any legacy submissions (`sitemap-index.xml`, old `lolcheats.org` URLs).
5. Use **URL Inspection** to request indexing for:
   - Homepage (`/`)
   - Pillar page (`/lol-cheats/`)
   - Key landing pages (`/lol-aimbot/`, `/lol-esp/`, `/lol-cheats-2026/`, etc.)
...
- [ ] Custom domain `lolcheats.org` attached and active
- [ ] `www` redirects to apex
- [ ] Legacy domains 301 to `lolcheats.org`
- [ ] Always Use HTTPS enabled
- [ ] `robots.txt` and sitemaps serve from `https://lolcheats.org`
- [ ] Google Search Console domain verified
- [ ] `sitemap.xml` submitted in GSC
- [ ] Homepage and `/lol-cheats/` requested for indexing
