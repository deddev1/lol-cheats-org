...
const assetsDir =
	'C:/Users/Aman/.cursor/projects/c-Users-Aman-lolhacks/assets';
...
const USER_SCREENSHOTS = [
	...
	},
	{
		...
		file: 'lol-wallhack-skeleton.webp',
	},
	{
		source:
...
async function removeOldLeague of LegendsImages() {
...
async function generateResponsiveVariants() {
	const heroSource = path.join(imagesDir, 'lol-esp-player-tags.webp');
	const heroMeta = await sharp(heroSource).metadata();

	for (const width of HERO_WIDTHS) {
		if (heroMeta.width && width > heroMeta.width) continue;
		const file = `lol-esp-player-tags-${width}w.webp`;
	...
	for (const { file } of USER_SCREENSHOTS) {
		if (file === 'lol-esp-player-tags.webp') continue;
...
async function generateFavicons() {
	const logoPath = path.join(imagesDir, 'lol-cheats-logo.png');
...
await removeOldLeague of LegendsImages();
