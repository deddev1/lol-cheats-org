...
/** Only publish guides tied to the cheat product — no generic gameplay filler. */
...
${cheatSources.map(finalizePost).map(buildPost).join(',\n')}
];
`;

writeFileSync(OUT, file);
console.log(`Wrote ${cheatSources.length} cheat-focused blog posts (${sources.length - cheatSources.length} skipped) → ${OUT}`);
